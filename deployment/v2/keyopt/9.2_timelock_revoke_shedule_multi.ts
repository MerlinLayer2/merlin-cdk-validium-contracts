...
async function main() {
    ...
    for (let i = 0; i < changeAdminRoles.length; i++) {
        ...
        const transactionResponse = await buildMultiSigBodyWithBody(deployer, scheduleData, parameters.timelockAddress, 'submitTransaction', keyPathParameters.new_timeLockKeyMultiSignerAddress);
        // eslint-disable-next-line no-await-in-loop

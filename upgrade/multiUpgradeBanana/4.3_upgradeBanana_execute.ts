...
//const pathUpgradeOutputJson = path.join(__dirname, "./upgrade_output.json");

const pathUpgradeOutputJson = require("./upgrade_output.json");

const fork12onfigPath = require("./merlin-upgrade-fork12.json")
...
async function main() {
    let deployerPath = fork12onfigPath.admin_path
...
async function buildMultiSigBodyWithBody(wallet: any){
    console.log("!!!! executeData ", pathUpgradeOutputJson.executeData)
    if (
        pathUpgradeOutputJson.executeData === undefined ||
        pathUpgradeOutputJson.executeData === ""
    ){
        console.log("executeData is empty, please input the executeData")
        throw new Error("executeData is empty, please input the executeData");
        return
    }
    ...
    const multiTimelockData = contractInterface3.encodeFunctionData(
        "submitTransaction",
        [
            fork12onfigPath.timelock_contract,
            0, pathUpgradeOutputJson.executeData
        ]
    );

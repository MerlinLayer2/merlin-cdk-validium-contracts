...
async function main() {
    let deployerPath = fork12configPath.timelock_path
...
async function buildMultiSigBodyWithBody1(wallet: any){
    if (
        fork12configPath.update_timelock_delay === undefined ||
        fork12configPath.update_timelock_delay === 0
    ){
        console.log("update_timelock_delay is empty, please input the update_timelock_delay")
        throw new Error("update_timelock_delay is empty, please input the update_timelock_delay");
        return
    }
    console.log("update_timelock_delay is", fork12configPath.update_timelock_delay)
    // @ts-ignore
    ...
    const updateDelayData = contractInterface1.encodeFunctionData(
        "updateDelay",
        [
            fork12configPath.update_timelock_delay
        ]
    );
    ...
    const timeLockData = contractInterface2.encodeFunctionData(
        "schedule",
        [
            fork12configPath.timelock_contract,
            0, updateDelayData, ethers.ZeroHash, fork12configPath.salt, fork12configPath.timelock_delay
        ]
    );
    ...
    let r = await wallet.sendTransaction({
        ...
        to: fork12configPath.multi_timelock,

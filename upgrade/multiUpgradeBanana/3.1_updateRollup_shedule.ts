...
async function buildMultiSigBodyWithBody1(wallet: any){
    //获取rollup type id
    const contract = new ethers.Contract(fork12configPath.polygonRollupManagerAddress, rollupManagerQueryABI, wallet);
    const rollupData = await contract.rollupIDToRollupData(1);
    let rollupTypeID = Number(rollupData[10]) + 1
    console.log("please confirm rollup type id is", rollupTypeID); // todo need confirm the rollup type id
    ...
    const addRollupData = contractInterface1.encodeFunctionData(
        ...
        [
            ...
            fork12configPath.polygonZkevmAddress,
            rollupTypeID,
            "0x"
        ]
    );
    ...
    const timeLockData = contractInterface2.encodeFunctionData(
        "schedule",
        [
            fork12configPath.polygonRollupManagerAddress,
            0, addRollupData, ethers.ZeroHash, fork12configPath.salt, fork12configPath.timelock_delay
        ]
    );
